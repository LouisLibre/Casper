...
struct NotchOverlaySpaceTests {
    ...
    static func main() {
        ...
        let overlayBehavior = panel.collectionBehavior
        ...
        expect(!panel.systemAlerts.isConfirming, "No confirmation is pending before it starts")
        panel.systemAlerts.beginConfirmation()
        expect(panel.systemAlerts.isConfirming, "Confirmation blocks focus before the modal window appears")
        settle()
        expect(reader.spaces(panel, mask: 15) == overlay, "Confirmation keeps body in overlay")
        expect(reader.spaces(band, mask: 15) == overlay, "Confirmation keeps band in overlay")
        expect(reader.spaces(panel, mask: 7).isEmpty, "Confirmation avoids the ordinary desktop handoff")
        expect(panel.collectionBehavior == overlayBehavior, "Confirmation preserves overlay Space flags")
        panel.systemAlerts.beginConfirmation()
        panel.systemAlerts.endConfirmation()
        expect(panel.systemAlerts.isConfirming, "Closing an inner confirmation still leaves the outer one pending")
        settle()
        expect(reader.spaces(panel, mask: 15) == overlay, "Nested confirmation keeps the overlay active")
        panel.systemAlerts.endConfirmation()
        expect(!panel.systemAlerts.isConfirming, "Closing the last confirmation allows focus restoration")
        settle()
        expect(reader.spaces(panel, mask: 15) == overlay, "Last confirmation closing preserves the overlay")
        expect(reader.spaces(band, mask: 15) == overlay, "Band stays in overlay after confirmation")
